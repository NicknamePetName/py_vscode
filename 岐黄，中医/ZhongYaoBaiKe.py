...
class_list = [] # 类名
# 获取需要爬取的药 
response = requests.get('https://www.qihuang.vip/yao.php',headers=headers)
bs_book = BeautifulSoup(response.text,'html.parser')
h2_tags = bs_book.find_all('h2')
for h2_tag in h2_tags:
    a = re.findall('.*?药',h2_tag.text,re.S) 
    b = ''
    class_list.append(b.join(a))
class_list.pop(0)


is_login = True # 登录，验证身份，,每次启动都要登录

for i in range(1,22):  #   这里改变的是第几大类0-11
    # 爬取的大类
    kind_name = class_list[i]
    class_book = f'category_{i}'
    ...
    for tr in tr_s:
        ...
        file_ = '中药百科' + '/'+ kind_name + '/' + tr_classify
